...
const { cycle, replicate, splitArray, zip, zipWith } = require('../arrays')
...
describe('array helpers', () => {
  ...
  describe('splitArray', () => {
    it('should split the array where you ask', () => (
      assert.deepEqual(
    ...
    it('should split the array left when the range is >= length', () => (
      assert.deepEqual(
        splitArray([1, 2, 3, 4, 5], 5),
        { head: [1, 2, 3, 4, 5], tail: [] }
      )
    ))

    it('should split the array right when the range is <= 0', () => (
      assert.deepEqual(
        splitArray([1, 2, 3, 4, 5], 0),
        { head: [], tail: [1, 2, 3, 4, 5] }
      )
    ))
  })
